...
function isGreaterThan (n1, n2) {
  ...
  return (
    parseInt(JSON.parse(JSON.stringify(n1, null, 4)), 10) >
...
function calcDelta (n1, n2) {
  // handling numbers in js is near impossible so we do this, which is ugly but seems to work
  if (
    parseInt(JSON.parse(JSON.stringify(n1, null, 4)), 10) < 0 &&
    parseInt(JSON.parse(JSON.stringify(n2, null, 4)), 10) < 0
  ) {
    return Math.abs(
      Math.abs(parseInt(JSON.parse(JSON.stringify(n1, null, 4)), 10)) -
        Math.abs(parseInt(JSON.parse(JSON.stringify(n2, null, 4)), 10))
    ).toFixed();
  } else {
    return Math.abs(
      Math.abs(parseInt(JSON.parse(JSON.stringify(n1, null, 4)), 10)) +
...
export function getDistance (pos1, pos2) {
  // handling numbers in js is near impossible so we do this, which is ugly but seems to work
  var deltaX =
    parseInt(JSON.parse(JSON.stringify(pos1.x, null, 4)), 10) -
    parseInt(JSON.parse(JSON.stringify(pos2.x, null, 4)), 10);
  var deltaY =
    parseInt(JSON.parse(JSON.stringify(pos1.y, null, 4)), 10) -
    parseInt(JSON.parse(JSON.stringify(pos2.y, null, 4)), 10);
  var deltaZ =
    parseInt(JSON.parse(JSON.stringify(pos1.z, null, 4)), 10) -
