...
class EnvironmentManager {
  ...
  constructor () {
    ...
    drawingContext.fillRect(
      0,
      0,
      this.sceneManager.groundSize.x,
      this.sceneManager.groundSize.y
    );
   ...
   *
   * The fill object will assign the values that match the keys in the
   * envConsumeKeys array to each tile in the Environment
   *
  ...
  updateWeatherModifier () {
    ...
    const weatherIdx = this.defaultEnvironment.weatherTypes.findIndex(
      type => type === newWeather
    );
  ...
  prettyPrintEnvStateToConsole (envParam) {
    ...
    console.clear();
    console.warn(
      "CAUTION: USE SPARINGLY, THIS CLEARS THE CONSOLE AND PUTS A SIGNIFICANT" +
  ...
  germinate (object) {
    ...
    if (this.checkIfLiving(object)) {
      if (object.germinationLevel >= 1.0) {
        ...
        object.germinationLevel +=
          (2 - this.weatherMod) * object.germinationRate;
      }
    } else {
  ...
  checkIfLiving (object) {
    ...
    return this.defaultEnvironment.envConsumeKeys.every(
      key => envAtObj[key] >= 0
    );
  }
   ...
   * Animal type objects are assigned only certain properties from
   * auxEnvParams based on the defaultEnvironment.numAnimalParams value.
   * numAnimalParams specifies the number of properties to be assigned to
   * animal type objects starting from the last position of
   * auxEnvParams.
   *
   * Objects are matched to their property values based on the object type.
   * It is presumed that within the defaultEnvironment object
   * there is a array called "{objectType}params" that has the object type in it's key.
   * These values are assigned to the objects
  ...
  registerTrackedObject (envObject) {
    ...
    const targetArrLength =
      this.defaultEnvironment.envConsumeKeys.length +
      this.defaultEnvironment.auxEnvParams.length;
    ...
    if (
      objectKey.length > 0 &&
      this.defaultEnvironment[objectKey].length === targetArrLength
    ) {
      ...
      for (var i = 0; i < this.defaultEnvironment.envConsumeKeys.length; i++) {
        envObject[
          this.defaultEnvironment.envConsumeKeys[i]
        ] = EnvironmentManager.getRandomByPercent(
          this.defaultEnvironment[objectKey][i],
          20
        );
      }
      ...
      for (var j = 0; j < this.defaultEnvironment.auxEnvParams.length; j++) {
        ...
        envObject[
          this.defaultEnvironment.auxEnvParams[j]
        ] = EnvironmentManager.getRandomByPercent(
          this.defaultEnvironment[objectKey][i + j],
          20
        );
      }
      ...
      objectKey.length > 0 &&
      this.defaultEnvironment[objectKey].length ===
        this.defaultEnvironment.numAnimalParams
    ) {
      const endOfAuxArr = this.defaultEnvironment.auxEnvParams.length - 1;

      for (var k = 0; k < this.defaultEnvironment.numAnimalParams; k++) {
        envObject[
          this.defaultEnvironment.auxEnvParams[endOfAuxArr - k]
        ] = EnvironmentManager.getRandomByPercent(
          this.defaultEnvironment[objectKey][k],
          20
        );
      }
      ...
      console.warn(
        "Object type: " +
          envObject.type +
          " is not currently supported by EnvironmentManger or\n" +
  ...
  getObjectParamKeyFromType (type) {
    var findKey = Object.keys(this.defaultEnvironment).find(key =>
  ...
  /**
   * Called in the onDestroy function of each object. The object's nutrientOnDeath
   * value is added to the environment tile the object was located on at death.
   * If the object has envConsumeKey properties, it is queued to be removed from the
  ...
  * localEnvGenerator () {
  ...
  /**
   * Iterates through all environment tiles that have a water value less
   * than this.defaultEnvironment.waterBalanceThreshold
   * and redistributes water to that tile from neighboring tiles
   * that have a value above this.defaultEnvironment.waterBalanceThreshold
   *
  ...
  async balanceWaterTable () {
    ...
    let lowWater = [...envGen].filter(
      val => val.env.water < this.defaultEnvironment.waterBalanceThreshold
    );

    for (var i = 0; i < lowWater.length; i++) {
      // Will return valid adjacent env tiles
      let neighborsWithWater = this.getAdjacentTiles(
        lowWater[i].x,
        lowWater[i].y
      ).filter(
      ...
      if (neighborsWithWater.length > 0) {
        let waterBalanced =
          this.defaultEnvironment.waterFlow / neighborsWithWater.length;
        for (var j = 0; j < neighborsWithWater.length; j++) {
          neighborsWithWater[j].water -= waterBalanced;
        }

        lowWater[i].env.water +=
          this.defaultEnvironment.waterFlow +
          this.weatherMod * this.defaultEnvironment.waterRegen;
      }
    }
  }
   ...
   *  Balances the water table and calls consume/germinate for each tracked object.
   * Any objects that died between updates
    ...
    if (simTime - this.envTime > this.defaultEnvironment.updateRate * 30) {
      ...
      if (this.objectCreationQueue.length > 0) {
        ...
        if (this.objectCreationQueue.length > 50) {
          ...
          for (let k = 0; k < targetGrassField.length; k++) {
